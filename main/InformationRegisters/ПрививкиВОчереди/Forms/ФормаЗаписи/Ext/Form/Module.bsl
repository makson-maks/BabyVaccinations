...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПроверятьНеобходимостьСмещения = Истина;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если Запись.ДатаНапоминания <> ДатаНапоминанияДоИзменений Тогда
	...
	Если ПроверятьНеобходимостьСмещения Тогда
		ПроверитьНеобходимостьСмещенияВсегоКалендаря(Истина);
	КонецЕсли;
...
Процедура ПодтвердитьВакцинацию(Команда)
	...
	Если Параметры.Ключ.Пустой() Или Модифицированность Тогда
		ПроверятьНеобходимостьСмещения = Ложь;
		Записать();
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Элементы.ОтменитьВакцинацию.Видимость = Не Параметры.Ключ.Пустой();
	
	Если Запись.ЕстьЗаписьКВрачу Тогда
		Элементы.ЕстьЗаписьКВрачу.Заголовок = "Есть запись к врачу";
	Иначе
		Элементы.ЕстьЗаписьКВрачу.Заголовок = "Отсутствует запись к врачу";
	КонецЕсли;
...
Процедура ПодтвердитьВакцинациюОтветНаВопрос(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ПроверитьНеобходимостьСмещенияВсегоКалендаря();
...
&НаКлиенте
Процедура ПроверитьНеобходимостьСмещенияВсегоКалендаря(ИсключитьИзСмещенияТекущуюЗапись = Ложь)
	...
	Если КоличествоДнейСмещения <> 0 Тогда
		...
		ДополнительныеПараметры.Вставить("ИсключитьИзСмещенияТекущуюЗапись", ИсключитьИзСмещенияТекущуюЗапись);
	...
	Иначе
		...
		ОтправитьОповещенияОбИзмененииЗаписиРегистра();
...
Процедура СмещениеКалендаряОтветНаВопрос(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		СместитьКалендарьПрививокСервер(Запись.Ребенок, 
			ДополнительныеПараметры.КоличествоДнейСмещения,
			ДополнительныеПараметры.ИсключитьИзСмещенияТекущуюЗапись);	
	КонецЕсли; 
...
&НаКлиенте
Процедура ОтправитьОповещенияОбИзмененииЗаписиРегистра()
	
	ОбновитьПовторноИспользуемыеЗначения();
	Оповестить("ОбновитьФормуНачальнойСтраницы");
	ОповеститьОбИзменении(Тип("РегистрСведенийКлючЗаписи.ПрививкиВОчереди"));
	Если Открыта() Тогда
		Закрыть();
	КонецЕсли;
...
&НаСервере
Процедура СместитьКалендарьПрививокСервер(Ребенок, КоличествоДнейСмещения, ИсключитьИзСмещенияТекущуюЗапись)

	СтрукутраРегистра = ОбщегоНазначенияСервер.СтруктураРегистраСведений("ПрививкиВОчереди");
	ЗаполнитьЗначенияСвойств(СтрукутраРегистра, Запись);
	
	РаботаСКалендаремСервер.СместитьКалендарьПрививок(Ребенок, СтрукутраРегистра, КоличествоДнейСмещения, ИсключитьИзСмещенияТекущуюЗапись);
